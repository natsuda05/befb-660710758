...
func createStudent(c *gin.Context) {
	...
	//Souldbind return เป็น error ให้
	...
	if newStudent.Name == "" {
		c.JSON(http.StatusBadRequest, gin.H{"erro": "name is required"})
		return
	}

	if newStudent.Year < 1 || newStudent.Year > 4 {
		c.JSON(http.StatusBadRequest, gin.H{"erro": "year must be 1-4"})
		return
	}
...
func deleteStudent (c *gin.Context){
	...
	c.JSON(http.StatusNotFound , gin.H{"error" : "student no found"})
